...
class PurchaseOrderService extends OdooService {
  ...
  Future<List<PurchaseOrderApprovalResponse>> getEmployeePurchaseOrderList(
    ...
    List<PurchaseOrderApprovalResponse> purchase_order_approval_list =
        new List<PurchaseOrderApprovalResponse>();
    if (response.statusCode == 200) {
      var list = response.data;
      if (list.length > 0) {
        list.forEach((v) {
          purchase_order_approval_list
              .add(PurchaseOrderApprovalResponse.fromMap(v));
        });
      }
    }
  ...
  Future<bool> approvePurchaseOrder(int id, String empID) async {
    ...
    Response response = await dioClient
        .put(url, data: jsonEncode({'employee_id': int.parse(empID)}))
  ...
  Future<bool> cancelPurchaseOrder(int id, String reason) async {
    ...
    String url = Globals.baseURL +
        ...
        "/action_refuse";
    Response response = await dioClient.put(url,
        data: jsonEncode({'reason': reason.toString()}));
