...
class OvertimeRequestResponse {
  ...
  factory OvertimeRequestResponse.fromMap(Map<String, dynamic> map) {
    ...
    return OvertimeRequestResponse(
        ...
        requested_employee_id:
            RequestedEmployee.fromMap(map['requested_employee_id']),
        department_ids: List<Department>.from(
            map['department_ids']?.map((x) => Department.fromMap(x))),
        request_line: List<RequestLine>.from(
            map['request_line']?.map((x) => RequestLine.fromMap(x))),
        categ_id: OTCategory.fromMap(map['categ_id']));
  }

  String toJson() => json.encode(toMap());

  factory OvertimeRequestResponse.fromJson(String source) =>
      OvertimeRequestResponse.fromMap(json.decode(source));
