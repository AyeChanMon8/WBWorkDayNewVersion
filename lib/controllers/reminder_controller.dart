...
class ReminderController extends GetxController {
  ...
  _getRewards() async {
    ...
    await employeeService
        .rewardList(employee_id, offset.toString())
        .then((data) {
      if (data.length != 0) {
  ...
  Future<void> getReminderApproval() async {
    ...
    var employee_id = box.read('emp_id');
    await employeeService
        .rewardApprovalList(employee_id, offset.toString())
        .then((data) {
      if (data.length != 0) {
        if (offset != 0) {
  ...
  Future<void> getReminderApprove() async {
    ...
    var employee_id = box.read('emp_id');
    await employeeService
        .rewardApproveList(employee_id, offset.toString())
        .then((data) {
      if (data.length != 0) {
        if (offset != 0) {
          isLoading.value = false;
  ...
  void downloadReward(Reward reward) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    File file =
        await employeeService.downloadReward(reward.id, 'reward${reward.id}');
