...
class InsuranceController extends GetxController {
  ...
  void onChangePolicyDropdown(Insurancetypemodel insurancetypemodel) async {
    this.selectedPolicyType.value = insurancetypemodel;
    update();
  }

  void onChangeInsuranceDropdown(Insurancemodel insurancetypemodel) async {
    this.selectedInsuranceType.value = insurancetypemodel;
    update();
  }

  void onChangeInsuranceRefDropdown(Insurancemodel insurancetypemodel) async {
  ...
  void calculateBalanceAmount() {
    ...
    if (insurancemodel.balanceAmount > 0)
      balanceAmount.value =
          insurancemodel.balanceAmount - int.parse(txtCalimAmont.text);
    else
      balanceAmount.value =
          insurancemodel.coverageAmount - int.parse(txtCalimAmont.text);
  }

  void createClaimInsurance() async {
    if (balanceAmount == 0 ||
        double.tryParse(txtCalimAmont.text.toString()) >
            double.tryParse(balanceAmount.toString())) {
      Get.snackbar('Warning', 'Can not Claim!',
      ...
      await employeeService
          .createClaimInsurance(
              selectedInsuranceType.value,
              int.parse(txtCalimAmont.text),
              txtDescription.text,
              image_base64,
              formattedDate,
              refList)
  ...
  void createEmployeensurance() async {
    ...
    await employeeService
        .createEmployeeInsurance(
            selectedPolicyType.value, int.parse(employee_id), employee_name)
        .then((data) {
      insuranceList.value = data;
      Get.back();
      Get.back();
