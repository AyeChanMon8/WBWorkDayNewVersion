...
//plantrip waybill
  ...
  Future<void> getPlantripWithWayBillList(String state) async {
    ...
    await planTripServie
      ...
      if (state == 'running') {
        ...
        //   // recalculate
        if (plantrip_with_waybill_list != null &&
            plantrip_with_waybill_list.length > 0) {
          calculateTotalExpense(
              plantrip_with_waybill_list[arg_index.value].expenseIds);
        }
  ...
  void saveRouteDate(int pID) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    await planTripServie
        .updateRouteDate(pID, fromDateTextController.text.toString(),
            toDateTextController.text.toString())
        .then((data) {
      if (data != 0) {
        AppUtils.showConfirmDialog('Information', 'Successfully Updated!', () {
          Get.back();
  ...
  void calculateinterval(DateTime selectedFromDate, DateTime selectedToDate) {
    if (fromDateTextController.text.isNotEmpty &&
      ...
      if (selectedToDate.isAfter(selectedFromDate) ||
          selectedToDate.isAtSameMomentAs(selectedFromDate)) {
      } else {
  ...
  clickWayBillRouteLine(bool first_route, int trip_id, int route_id,
      int next_route_id, int previous_route_id) async {
    Future.delayed(
        Duration.zero,
        () => Get.dialog(
            Center(
    ...
    var created = 0;
    await planTripServie
        .clickWayBillRouteLineTrip(
            first_route, trip_id, route_id, next_route_id, previous_route_id)
        .then((data) {
      if (data != 0) {
        Get.back();
        created = 1;
  ...
  clickProductRouteLine(bool first_route, int trip_id, int route_id,
      int next_route_id, int previous_route_id) async {
    Future.delayed(
        Duration.zero,
        () => Get.dialog(
  ...
  void clickstartPlanTripProduct(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await planTripServie.startPlanTripProductTrip(id.toString()).then((data) {
      if (data != 0) {
  ...
  void clickendPlanTripProduct(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await planTripServie.endPlanTripProductTrip(id.toString()).then((data) {
      if (data != 0) {
        AppUtils.showConfirmDialog('Information', 'Successfully Ended!', () {
          Get.back();
          Get.back();
  ...
  void clickstartPlanTripWayBill(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await planTripServie.startPlanTripWaybillTrip(id.toString()).then((data) {
      if (data != 0) {
        AppUtils.showConfirmDialog('Information', 'Successfully Started!', () {
          Get.back();
          Get.back();
  ...
  void clickendPlanTripWayBill(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await planTripServie.endPlanTripWaybillTrip(id.toString()).then((data) {
      if (data != 0) {
        AppUtils.showConfirmDialog('Information', 'Successfully Ended!', () {
          Get.back();
          Get.back();
