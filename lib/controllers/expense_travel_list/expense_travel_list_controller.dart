...
class ExpensetravelListController extends GetxController {
  ...
  submitTravelExpense(int id, BuildContext context,
      List<TravelLineListModel> tLine, int employee_id, String code) async {
    Future.delayed(
        Duration.zero,
        () => Get.dialog(
            Center(
    ...
    await travelRequestService
        .submitTravelExpense(id, tLine, employee_id, code)
        .then((data) {
      Get.back();
  ...
  Future<void> getExpenseListForEmp() async {
    ...
    if (travelRequestService == null)
      this.travelRequestService = await TravelRequestService().init();
    await travelRequestService
        .getTravelExpenseModel(employee_id, offset.toString())
        .then((data) {
      if (offset != 0) {
