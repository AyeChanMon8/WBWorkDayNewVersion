...
class BusinessTravelUpdateController extends GetxController {
  ...
  Rx<Fleet_model> _selectedEditVehicle = Fleet_model().obs;
  Fleet_model get selectedEditVehicle => _selectedEditVehicle.value;
  ...
  Rx<TravelExpenseProduct> _selectedEditExpenseProduct =
      TravelExpenseProduct().obs;
  ...
  addTravelLine(List imageList) {
    ...
    } else if (selectedExpenseCategory.is_vehicle_selected != null &&
        selectedExpenseCategory.is_vehicle_selected) {
    ...
    if (valid) {
      ...
      var travelExpense = TravelLineModel(
          ...
          image8: image9,
          image9: image10,
          attachment_include: attach_include);
  ...
  updateTravelLineExpense(
      int id, int travel_id, String code, BuildContext context) async {
    var employee_id = int.tryParse(box.read('emp_id'));
    ...
    if (valid) {
      ...
      TravelExpenseUpdateModel travelExpense =
          TravelExpenseUpdateModel(id: id, travel_line: travelLineModel.value);
  ...
  void editCalculateAmount() {
    if (editPriceController.text.isNotEmpty &&
        editQtyController.text.isNotEmpty) {
      amount.value = double.tryParse(editQtyController.text) *
      ...
      update();
    } else if (editTotalAmountController.text.isNotEmpty &&
        editQtyController.text.isNotEmpty) {
      amount.value = double.tryParse(editTotalAmountController.text) /
          double.tryParse(editQtyController.text);
  ...
  void editCalculateUnitAmount() {
    if (editTotalAmountController.text.isNotEmpty &&
        editQtyController.text.isNotEmpty) {
      amount.value = double.tryParse(editTotalAmountController.text) /
          double.tryParse(editQtyController.text);
      editPriceController.text = amount.value.toStringAsFixed(2);
  ...
  Future<void> getOneTravelApprove(int travel_id) async {
    var employee_id = int.tryParse(box.read('emp_id'));
    await masterService
        .getOneTravelApprove(employee_id, travel_id)
        .then((data) {
      if (data != null) {
        from_travel_date.value = data[0].start_date;
        to_travel_date.value = data[0].end_date;
  ...
  setEditExpenseLine(TravelLineModel travelLine) {
    ...
    if (travelLine.categ_id != null) {
      for (var i = 0; i < this.edit_travel_expense_category_list.length; i++) {
        if (travelLine.categ_id ==
            this.edit_travel_expense_category_list[i].id) {
          this.selectedEditExpenseCategory =
              this.edit_travel_expense_category_list[i];
          this.selectedEditExpenseCategory.id =
              this.edit_travel_expense_category_list[i].id;
          break;
        }
      }
  ...
  void onChangeEditVehicleDropdown(Fleet_model fleet_model) async {
  ...
  updateExpenseLine(int index, TravelLineModel travelLine, List<String> img) {
    ...
    if (expenseUpdateDateController.text.isEmpty) {
      ...
      valid = false;
      AppUtils.showDialog("Information!", "Approve Travel List is needed!");
    } else if (selectedEditExpenseCategory.is_vehicle_selected != null &&
        selectedEditExpenseCategory.is_vehicle_selected) {
