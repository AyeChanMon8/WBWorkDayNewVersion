...
class OvertimeRequestController extends GetxController {
  ...
  OvertimeCategory get selectedOvertimeCategory =>
      _selectedOvertimeCategory.value;
      ...
      if (!dept_found) {
        ...
        await overtimeService
          ...
          for (int i = 0; i < data.length; i++) {
            ...
            var from_date = DateFormat('yyyy-MM-dd HH:mm')
                .parse(selectedFromDate.toString().split('.')[0])
                .subtract(Duration(hours: 6, minutes: 30))
                .toString()
                .split('.')[0];
            var to_date = DateFormat('yyyy-MM-dd HH:mm')
                .parse(selectedEndDate.toString().split('.')[0])
                .subtract(Duration(hours: 6, minutes: 30))
                .toString()
                .split('.')[0];
    ...
    await masterService.getDepartmentList(employee_id).then((data) {
      ...
      data.insert(
          ...
          OTDepartment(
              id: 0,
              name: 'Department',
              branch_id: Branch_id(id: 0, name: 'Branch')));
  ...
  void calculateinterval(DateTime selectedFromDate, DateTime selectedToDate) {
    if (fromDateTimeTextController.text.isNotEmpty &&
      ...
      if (selectedToDate.isAfter(selectedFromDate)) {
        final differenceHr =
            selectedToDate.difference(selectedFromDate).inHours;
        final differenceHalf = differenceHr + 0.5;
        final difference =
            selectedToDate.difference(selectedFromDate).inMinutes;
