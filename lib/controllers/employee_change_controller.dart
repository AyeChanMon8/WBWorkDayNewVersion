...
class EmployeeChangeController extends GetxController {
  ...
  set selectedManagerEmployee(Emp_job type) =>
      _selectedManagerEmployee.value = type;
  ...
  set selectedOldDepartment(Department type) =>
      _selectedOldDepartment.value = type;
  ...
  getEmployeeChangeList() async {
    ...
    await employeeService
        .getEmployeeChangeList(int.tryParse(employee_id), offset.toString())
        .then((data) {
  ...
  getEmployeeList(int companyID, int branchId, int deptID) async {
    ...
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    await employeeService
        .getAllEmployeeList(companyID, branchId, deptID)
        .then((value) {
      if (value.length != 0) {
  ...
  getManagerEmployeeList(int companyID, int branchId, int deptId) async {
    ...
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    await employeeService
        .getManagerEmployeeList(companyID, branchId, deptId)
        .then((data) {
      if (data.length != 0) {
  ...
  getEmployeeJobInfo() async {
    ...
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    await employeeService
        .getEmployeeJobInfo(this.selectedEmployee.id)
        .then((data) {
      this._selectedEmployeeJobInfo.value = data;
      this.old_salary_level.value =
          AppUtils.removeNullString(data.salary_level);
      this.old_jobgrade.value = AppUtils.removeNullString(data.job_grade);
      this.old_salary_level_name.value =
          AppUtils.removeNullString(data.salary_level_name);
      this.old_jobgrade_name.value =
          AppUtils.removeNullString(data.job_grade_name);
  ...
  getNewDepartmentList(int id) async {
    ...
    await employeeService.getDepartmentList(id, "").then((data) {
      ...
      getManagerEmployeeList(
          this.selectedCompany.id, selectedBranch.id, selectedDepartment.id);
      Get.back();
    });
  }
  ...
  getOldDepartmentList(int id) async {
    ...
    await employeeService.getAllDepartmentList(id).then((data) {
      ...
      old_department_list.value = data;
      getEmployeeList(selectedOldCompany.id, selectedOldBranch.id,
          selectedOldDepartment.id);
      Get.back();
    });
  }
  ...
  getNewWage() async {
    ...
    await employeeService
        .getEmployeeNewWage(int.tryParse(this.selectedJobGrade.id.toString()),
            int.tryParse(this.selectedSalaryLevel.id.toString()))
        .then((data) {
      // print("newWage#");
  ...
  void onChangeDepartmentDropdown(Department value, String status) {
    if (status == "old") {
      ...
      update();
      getEmployeeList(this.selectedOldCompany.id, this.selectedOldBranch.id,
          this.selectedOldDepartment.id);
    } else {
      this.selectedDepartment = value;
      update();
      getManagerEmployeeList(this.selectedCompany.id, this.selectedBranch.id,
  ...
  void saveEmployeeChanges() async {
    ...
    await employeeService
        .createEmployeeChange(employee_change_create)
        .then((data) {
      print("created");
      Get.back();
