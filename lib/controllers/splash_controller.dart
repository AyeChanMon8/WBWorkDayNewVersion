...
class SplashController extends GetxController {
  ...
  void apiForRememberLogin() async {
    ...
    await employeeService.checkLogin(username, password).then((data) {
      if (!data.isNull) {
  ...
  void checkRole(String emp_id) async {
    ...
    await employeeService.checkRole(int.tryParse(emp_id)).then((data) async {
      ...
      if (value) if (storage.getItem('home_function') == null) {
      ...
      box
          .write('role_category', data)
          .then((value) => Get.offAll(BottomNavigationWidget()));
