...
class DayTripExpenseController extends GetxController {
  ...
  fetchExpenseStatusData(int tripID) async {
    ...
    this.dayTripServie = await DayTripServie().init();
    await dayTripServie
        .fetchExpenseStatusDayTipPlantrip(tripID, 'daytrip')
        .then((data) {
      print("BackClose#");
  ...
  addExpense() async {
    ...
    if (this.selectedExpenseType.id == 0 ||
      ...
      AppUtils.showConfirmCancelDialog('Warning', 'Are you sure?', () async {
        var expense = Expense(
            image: image_base64,
            productId: this.selectedExpenseType.id,
            name: descriptionController.text,
            amount: double.tryParse(totalAmountController.text),
            day_trip_id: daytrip_id);
        Future.delayed(
            Duration.zero,
            () => Get.dialog(
                Center(
        ...
        await dayTripServie
            .addExpense(
                expense, daytrip_id.toString(), int.tryParse(employee_id))
            .then((data) {
          image_base64 = "";
          // if (data != 0) {
          expense_list.add(DayTrip_Expense_ids(
              productId: DayTrip_Product_id(
                  id: this.selectedExpenseType.id,
                  name: this.selectedExpenseType.name),
              name: descriptionController.text,
              amount: double.tryParse(totalAmountController.text)));
  ...
  addFuelConsumption(DayTripModel dayTripModel, int lineId) async {
    if (actualFuelLiterTextController.text.isEmpty) {
      ...
      AppUtils.showConfirmCancelDialog('Warning', 'Are you sure?', () async {
        var employee_id = box.read('emp_id');
        var data = Daytrip_fuel_consumption(
            dayTripId: daytrip_id,
            consumedLiter: double.parse(actualFuelLiterTextController.text),
  ...
  void clickstartDayTrip(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await dayTripServie.startDayTrip(id.toString()).then((data) {
      if (data != 0) {
  ...
  void clickendDayTrip(int id) async {
    Future.delayed(
        ...
        () => Get.dialog(
            ...
            barrierDismissible: false));
    var employee_id = box.read('emp_id');
    await dayTripServie.endDayTrip(id.toString()).then((data) {
      if (data != 0) {
        AppUtils.showConfirmDialog('Information', 'Successfully Ended!', () {
          Get.back();
          Get.back();
