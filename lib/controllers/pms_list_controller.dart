...
class PmsListController extends GetxController {
  ...
  Future<List<PMSDetailModel>> getPmsList() async {
    ...
    await pmsService
        .getPmSelfList(int.tryParse(employee_id), offset.toString())
        .then((data) {
      if (offset != 0) {
  ...
  Future<List<PMSDetailModel>> getPmsApprovalList() async {
    ...
    var employee_id = box.read('emp_id');
    await pmsService
        .getPmsToApproveList(int.tryParse(employee_id), offset.toString())
        .then((value) {
      if (offset != 0) {
        // update data and loading status
        isLoading.value = false;
  ...
  Future<List<PMSDetailModel>> getPmsApprovedList() async {
    ...
    var employee_id = box.read('emp_id');
    await pmsService
        .getPmsApprovedList(int.tryParse(employee_id), offset.toString())
        .then((value) {
      if (offset != 0) {
        // update data and loading status
        isLoading.value = false;
