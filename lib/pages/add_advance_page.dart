...
//final DayTripExpenseController day_trip_controller = Get.find();
  DayTripPlanTripGeneralController day_trip_controller =
      Get.put(DayTripPlanTripGeneralController());
  var from_where = "";
  var arg_id = 0;
  var advanceTotal = 0.0;
  var advanceLineTotal = 0.0;
  AddAdvancePage(String from, int daytrip_id, double advanceTotal,
      double advanceLineTotal) {
    arg_id = daytrip_id;
    from_where = from;
              ...
              Obx(
                () => Container(
                  child: GFButton(
                    ...
                    onPressed: day_trip_controller.isAdvanceButton.value
                        ? null
                        : () {
                            advanceLineTotal = double.tryParse(
                                day_trip_controller.totalAmountController.text
                                    .toString());
                            if (advanceTotal >= advanceLineTotal)
                              day_trip_controller.addAdvance(
                                  from_where, arg_id);
                            else
                              AppUtils.showDialog('Warning!',
                                  'Total advance must not exceed allowed advance!');
                          },
                    text: "Save",
                    blockButton: true,
                    size: GFSize.LARGE,
                    ...
                    () => DropdownButtonHideUnderline(
                      child: DropdownButton<Daytrip_advance_expense_category>(
                          ...
                          onChanged: (Daytrip_advance_expense_category value) {
                            day_trip_controller
                                .onChangeExpenseCategoryForAdvanceDropdown(
                                    value);
                          },
