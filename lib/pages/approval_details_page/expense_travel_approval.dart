...
class _TravelExpenseDetailsState extends State<TravelExpenseApproval> {
  ...
  Widget build(BuildContext context) {
    ...
    index = Get.arguments;
    controller
        .getAttachmentTravel(controller.travelExpenseList.value[index].id);
    return Scaffold(
        appBar: AppBar(
          title: Text(
            ...
            children: [
              ...
              Obx(
                () => controller.showDetails.value
                    ? controller.travelExpenseList.length > 0
                        ? Column(
              ...
              Align(
                  ...
                  child: Container(
                      ...
                      child: Text(
                        '${labels?.total} Advance Amount : ${NumberFormat('#,###.#').format(controller.getTotalExpenseTravelAdvanceAmount(index))}',
                        style:
              ...
              Align(
                  ...
                  child: Container(
                      margin: EdgeInsets.symmetric(horizontal: 18),
                      child: Text(
                        '${labels?.total} Amount : ${NumberFormat('#,###.#').format(controller.getTotalExpenseTravelAmount(index))}',
                        style:
                            TextStyle(fontSize: 16, color: Colors.deepPurple),
                      ))),
              Divider(
              ...
              Align(
                  ...
                  child: Container(
                      margin: EdgeInsets.symmetric(horizontal: 18),
                      child: controller.getTotalRemainingAmount(index) < 0
                          ? Text(
                              'Surplus to be Returned : ${NumberFormat('#,###.#').format(controller.getTotalRemainingAmount(index).abs())}',
                              style: TextStyle(
                                  fontSize: 16, color: Colors.deepPurple),
                            )
                          : Text(
                              'Deficit for Reimbursement : ${NumberFormat('#,###.#').format(controller.getTotalRemainingAmount(index))}',
                              style: TextStyle(
                                  fontSize: 16, color: Colors.deepPurple),
                            ))),
              SizedBox(
              ...
              Obx(() => controller.travelExpenseList.length > 0
                  ? (controller.travelExpenseList.value[index].state !=
                          'approve'
              ...
              Container(
                child: Row(
                  ...
                  children: [
                    ...
                    //     )
                    // ):SizedBox() ,
                    controller.travelExpenseList[index].travel_line[ind]
                                    .attachment_include !=
                                null &&
                            controller.travelExpenseList[index].travel_line[ind]
                                .attachment_include
                ...
                child: controller.travelExpenseList.value[index].date != null
                    ? Text(
                        AppUtils.changeDateFormat(
                            controller.travelExpenseList.value[index].date),
                        style: subtitleStyle())
                    : Text('-'),
              ),
            ],
  ...
  Widget imageGridView(List<String> value) {
    return GridView.builder(
        ...
        itemBuilder: (context, fileIndex) => Container(
              child: Card(
                child: Image.memory(base64Decode(value[fileIndex]),
                    width: 80, height: 50),
              ),
            ));
  }
